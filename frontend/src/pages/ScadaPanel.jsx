...
function normalizeGate(gate) {
  const positionPercent = Number(
    gate.positionPercent
      ?? gate.opening_percentage
      ?? gate.open_percentage
      ?? gate.position_percent
      ?? 0,
  );
...
export default function ScadaPanel() {
  ...
  const [isGateEndpointMissing, setIsGateEndpointMissing] = useState(false);
  ...
  useEffect(() => {
    ...
    function applyLiveData({
      ...
      const responses = [liveResponse, sensorsResponse, turbinesResponse, gatesResponse].filter(Boolean);
      ...
      const nextGates = normalizeList(gatesResponse?.data, normalizeGate)
        ?? normalizeList(liveData.gates, normalizeGate);
    ...
    async function refreshLiveData({ includeAuxiliaryData = false } = {}) {
      ...
      try {
        let liveResponse;
        let turbinesResponse = null;
        let sensorsResponse = null;
        let gatesResponse = null;
        ...
        if (includeAuxiliaryData) {
          ...
          liveResponse = liveResult.value;
          turbinesResponse = turbinesResult.status === "fulfilled" ? turbinesResult.value : null;
          sensorsResponse = sensorsResult.status === "fulfilled" ? sensorsResult.value : null;

          if (gatesResult.status === "fulfilled") {
            gatesResponse = gatesResult.value;
            setIsGateEndpointMissing(false);
          } else if (gatesResult.reason?.status === 404) {
            setIsGateEndpointMissing(true);
          } else {
            setGateError(gatesResult.reason?.message || "Savak kapaklari yuklenemedi.");
          }
        } else {
          liveResponse = await getLiveScadaData();
        }
  ...
  return (
    <div className="space-y-6" data-testid="scada-panel">
      ...
      <GateControl
        ...
        isEndpointMissing={isGateEndpointMissing}
